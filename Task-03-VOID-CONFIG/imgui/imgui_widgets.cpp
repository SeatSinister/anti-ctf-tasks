...
struct checkbox_animation {
    float animation, opacity, checkmark, text;
};
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    it_anim->second.animation = ImLerp(it_anim->second.animation, *v ? 0.1f : 8.0f, 0.05f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.checkmark = ImLerp(it_anim->second.checkmark, *v ? 0.1f : 8.0f, 0.05f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.opacity = ImLerp(it_anim->second.opacity, *v ? 1.0f : 0.0f, 0.1f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text = ImLerp(it_anim->second.text, *v ? 0.8f : 0.4f, 0.1f * (1.0f - ImGui::GetIO().DeltaTime));
    ...
    window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, ImColor(30, 30, 30), 5.0f); 
    window->DrawList->AddRect(check_bb.Min + ImVec2(1, 1), check_bb.Max - ImVec2(1, 1), ImColor(40, 40, 40), 5.0f);
    {
        ImVec4 fill = GetStyleColorVec4(ImGuiCol_SliderGrab);
        fill.w *= it_anim->second.opacity;
        window->DrawList->AddRectFilled(check_bb.Min + ImVec2(it_anim->second.animation, it_anim->second.animation), check_bb.Max - ImVec2(it_anim->second.animation, it_anim->second.animation), ImGui::ColorConvertFloat4ToU32(fill), 5.0f);
    }

    if (*v)
    {
        const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
        RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad) + ImVec2(1, it_anim->second.checkmark + 1), ImColor(255, 255, 255), square_sz - pad * 3.0f);
    }

    ImVec2 label_pos = ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePadding.y);

    if (label_size.x > 0.0f)
        window->DrawList->AddText(label_pos, (ImColor)ImVec4(1.0f, 1.0f, 1.0f, it_anim->second.text), label);
...
struct begincombo_animation {
    float text, circle, open;
};
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImRect total_bb(bb.Min, ImVec2(window->DC.CursorPos.x + window->Size.x - 20, bb.Max.y + 5));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    static std::map<ImGuiID, begincombo_animation> anim;
    auto it_anim = anim.find(id);
    if (it_anim == anim.end()) {
        anim.insert({ id, {0.0f} });
        it_anim = anim.find(id);
    ...
    it_anim->second.open = ImLerp(it_anim->second.open, popup_open ? 1.0f : 0.0f, 0.04f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.circle = ImLerp(it_anim->second.circle, popup_open ? 1.0f : 0.1f, 0.1f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text = ImLerp(it_anim->second.text, popup_open ? 0.6f : 0.3f, 0.1f * (1.0f - ImGui::GetIO().DeltaTime));

    window->DrawList->AddRectFilled(total_bb.Min, total_bb.Max, ImColor(35, 35, 35), 5.0f);
    window->DrawList->AddRect(total_bb.Min, total_bb.Max, ImColor(50,50,50), 5.0f);
    // Custom preview
    ...
    window->DrawList->AddCircle(total_bb.Max - ImVec2(15, 13), 7.0f, ImColor(45, 45, 45), 100);
    {
        ImVec4 dot = GetStyleColorVec4(ImGuiCol_SliderGrab);
        dot.w *= it_anim->second.circle;
        const ImU32 dot_col = ImGui::ColorConvertFloat4ToU32(dot);
        window->DrawList->AddCircleFilled(total_bb.Max - ImVec2(15, 13), 7.0f, dot_col, 100);
        window->DrawList->AddShadowCircle(total_bb.Max - ImVec2(15, 13), 5.f, dot_col, 50.f, ImVec2(0, 0), 0, 12);
    }

    PushStyleColor(ImGuiCol_Text, (ImU32)(ImColor)ImVec4(1.0f, 1.0f, 1.0f, it_anim->second.text));
    RenderTextClipped(total_bb.Min + ImVec2(5, 5), total_bb.Max - ImVec2(27, 5), preview_value, NULL, NULL);
    PopStyleColor();

    if (label_size.x > 0 && smallfont)
    {
        ImGui::PushFont(smallfont);
        window->DrawList->AddText(total_bb.Min + ImVec2(1, -14), (ImColor)ImVec4(1.0f, 1.0f, 1.0f, it_anim->second.text), label);
        /*RenderText(ImVec2(total_bb.Min.x + 1, total_bb.Min.y - 14), label);*/
        ImGui::PopFont();
    }
    ...
    // Set popup size
    float widght = total_bb.GetWidth();
    if (g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasSizeConstraint)
    {
        g.NextWindowData.SizeConstraintRect.Min.x = ImMax(g.NextWindowData.SizeConstraintRect.Min.x, widght);
    }
    else
    {
        ...
        SetNextWindowSizeConstraints(ImVec2(widght, 0.0f), ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(popup_max_height_in_items) * it_anim->second.open));
    }
    ...
    PushStyleColor(ImGuiCol_PopupBg, ImVec4(ImColor(35, 35, 35)));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f) + ImVec2(60, 15));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(1, 22), frame_bb.Max - ImVec2(5, 6), ImColor(25, 25, 25), 5.0f);
    window->DrawList->AddRect(frame_bb.Min + ImVec2(1, 22), frame_bb.Max - ImVec2(5, 6), ImColor(35, 35, 35), 5.0f);
    ...
    it_anim->second.animation = ImLerp(it_anim->second.animation, grab_bb.Min.x  - frame_bb.Min.x , 0.1f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.opacity = ImLerp(it_anim->second.opacity, IsItemActive() ? 0.8f : 0.4f, 0.1f * (1.0f - ImGui::GetIO().DeltaTime));

    // Render grab (fill uses theme — was hardcoded pink ImColor(254,71,165))
    const ImU32 fill_col = GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab);
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(0, 23), frame_bb.Min + ImVec2(it_anim->second.animation, 30), fill_col, 5.0f);
    window->DrawList->AddCircleFilled(ImVec2(frame_bb.Min.x + it_anim->second.animation, frame_bb.Min.y + 26.5f), 6.f, ImColor(255, 255, 255), 55);
    ...
    if (g.LogEnabled)
        LogSetNextTextDecoration("{", "}");
    ImVec2 text_size = CalcTextSize(value_buf, value_buf_end, false);
    ImVec2 text_pos = frame_bb.Max - ImVec2(17, 27) - text_size * 0.5f;
    window->DrawList->AddText(text_pos, (ImColor)ImVec4(1.0f, 1.0f, 1.0f, it_anim->second.opacity), value_buf, value_buf_end);

    if (label_size.x > 0.0f)
        window->DrawList->AddText(frame_bb.Min - ImVec2(0, 1), (ImColor)ImVec4(1.0f, 1.0f, 1.0f, it_anim->second.opacity), label);
...
bool ImGui::Keybind(const char* label, int* key, bool show_label)
{
    ...
    static std::map<ImGuiID, keybind_animation> anim;
    auto it_anim = anim.find(id);
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, hovered ? 0.6f : 0.3f, 0.1f * (1.0f - ImGui::GetIO().DeltaTime));

    window->DrawList->AddRectFilled(clickable.Min + ImVec2(0, 0), clickable.Max - ImVec2(0, 0), ImColor(28,28,28), 3.0f);
    window->DrawList->AddRect(clickable.Min + ImVec2(0, 0), clickable.Max - ImVec2(0, 0), ImColor(35,35,35), 3.0f);

    if (show_label) window->DrawList->AddText(rect.Min + ImVec2(3, 1), (ImColor)ImVec4(1.0f, 1.0f, 1.0f, it_anim->second.text), label);
...
//  doing UTF8 > U16 > UTF8 conversions on the go to easily interface with stb_textedit. Ideall
// 
//y should stay in UTF-8 all the time. See https://github.com/nothings/stb/issues/188)

struct inputtext_animation {
    float opacity, move_one_x, move_one_y, move_two_x, move_two_y;
};
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImVec2 total_size = ImVec2(window->Size.x - 20, frame_size.y + 2);

    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + total_size);
    ...
    static std::map<ImGuiID, inputtext_animation> anim;
    auto it_anim = anim.find(id);
    if (it_anim == anim.end()) {
        anim.insert({ id, {0.0f} });
        it_anim = anim.find(id);
    ...
    it_anim->second.opacity = ImLerp(it_anim->second.opacity, IsItemActive() ? 0.8f : 0.3f, 0.06f * (1.0f - ImGui::GetIO().DeltaTime));
    ...
    if (!is_multiline)
    {
        ...
        window->DrawList->AddRectFilled(total_bb.Min, total_bb.Max, ImColor(32, 32, 32), 7.0f);
        window->DrawList->AddRect(total_bb.Min, total_bb.Max, ImColor(42, 42, 42), 7.0f);
    }
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            const ImVec4 sg_sel = GetStyleColorVec4(ImGuiCol_SliderGrab);
            ImU32 bg_color = GetColorU32(ImVec4(sg_sel.x, sg_sel.y, sg_sel.z, 0.12f), render_cursor ? 1.0f : 0.6f);
        ...
        if (render_cursor)
        {
            ...
            it_anim->second.move_one_x = ImLerp(it_anim->second.move_one_x, cursor_screen_rect.Min.x, 0.2f * (1.0f - ImGui::GetIO().DeltaTime));
            it_anim->second.move_one_y = ImLerp(it_anim->second.move_one_y, cursor_screen_rect.Min.y, 0.2f * (1.0f - ImGui::GetIO().DeltaTime));

            it_anim->second.move_two_x = ImLerp(it_anim->second.move_two_x, cursor_screen_rect.GetBL().x, 0.2f * (1.0f - ImGui::GetIO().DeltaTime));
            it_anim->second.move_two_y = ImLerp(it_anim->second.move_two_y, cursor_screen_rect.GetBL().y, 0.2f * (1.0f - ImGui::GetIO().DeltaTime));
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(ImVec2(it_anim->second.move_one_x, it_anim->second.move_one_y) + ImVec2(0, 0), ImVec2(it_anim->second.move_two_x, it_anim->second.move_two_y) + ImVec2(0, 3), ImColor(85, 85, 85), 1.0f);
    ...
    {
        ImGui::PushFont(smallfont);
        window->DrawList->AddText(frame_bb.Min + ImVec2(3,-14), (ImColor)ImVec4(1.0f, 1.0f, 1.0f, it_anim->second.opacity), label);
        ImGui::PopFont();
    }
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton("##ColorButton", col_v4, flags, ImVec2(25,0), ImVec2(CalcItemWidth()+ 18, -8)))
        {
        ...
        SetNextWindowSize(ImVec2(230, 210));
        ImGui::PushStyleVar(ImGuiStyleVar_PopupRounding, 5.0f);
        ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(10,10));
        ImGui::PushStyleColor(ImGuiCol_PopupBg, (ImU32)ImColor(35, 35, 35));
    ...
    static std::map<ImGuiID, coloredit_animation> anim;
    auto it_anim = anim.find(id);
    if (it_anim == anim.end()) {
        anim.insert({ id, {0.0f} });
        it_anim = anim.find(id);
    }
    it_anim->second.text = ImLerp(it_anim->second.text, IsPopupOpen("picker") ? 0.8f : 0.3f, 0.1f * (1.0f - ImGui::GetIO().DeltaTime));
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        window->DrawList->AddText(window->DC.CursorPos + ImVec2(7,0),(ImColor)ImVec4(1.0f, 1.0f, 1.0f, it_anim->second.text), label);
    }
...
// FIXME: this is trying to be aware of style.Alpha but not fully correct. Also, the color wheel will have overl
// 
// apping glitches with (style.Alpha < 1.0)
struct picker_state
{
    float hue_bar;
    float alpha_bar;
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    // Render SV Square
    GetForegroundDrawList()->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size - 80), GetColorU32(ImGuiCol_PopupBg), col_white, hue_color32, hue_color32, col_white, 5.0f, ImDrawCornerFlags_All);
    GetForegroundDrawList()->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size - 80), GetColorU32(ImGuiCol_PopupBg), 0, 0, col_black, col_black, 5.0f, ImDrawCornerFlags_All);
    ...
    GetForegroundDrawList()->AddCircle(it_anim->second.circle_move + bar_pos + ImVec2(0, 1), it_anim->second.circle, IM_COL32(50, 50, 50, (int)(150 * g.Style.Alpha)), 32);
    GetForegroundDrawList()->AddCircle(it_anim->second.circle_move + bar_pos, it_anim->second.circle, IM_COL32(230, 230, 230, (int)(255 * g.Style.Alpha)), 32);
    ...
    if (alpha_bar)
    {
        ...
        GetForegroundDrawList()->AddRectFilledMultiColorRounded(bar1_bb.Min + ImVec2(0, 8), bar1_bb.Max - ImVec2(0, 5), GetColorU32(ImGuiCol_PopupBg), 0, 0, 0, 0, 5.0f, ImDrawCornerFlags_All);
        ...
        GetForegroundDrawList()->AddCircleFilled(ImVec2(it_anim->second.alpha_bar + bar_pos.x, bar1_bb.Min.y + 13.0f), 6, IM_COL32(255, 255, 255, 255), 100);
    }
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg, const ImVec2& pos_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImRect bb(window->DC.CursorPos + pos_arg, window->DC.CursorPos + size + ImVec2(10, 0) + pos_arg);
    ...
    GetWindowDrawList()->AddShadowRect(bb_inner.Min, bb_inner.Max + ImVec2(0, 0), GetColorU32(col_rgb), 20, ImVec2(-1.f, -1.f), NULL, 32);
...
// FIXME: Selectable() with (size.x == 0.0f) and (SelectableTextAlign.x > 0.0f) followed by SameLine()
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (hovered || selected)
    {
        ...
        window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(30, 30, 30), 10.0f);
        window->DrawList->AddRect(bb.Min, bb.Max, ImColor(35, 35, 35), 10.0f);
    }
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, (hovered || selected) ? 0.85f : 0.45f, 0.1f * (1.0f - ImGui::GetIO().DeltaTime));
    {
        const ImVec4 c_on = GetStyleColorVec4(ImGuiCol_Text);
        const ImVec4 c_off = GetStyleColorVec4(ImGuiCol_TextDisabled);
        it_anim->second.r = ImLerp(it_anim->second.r, selected ? c_on.x : c_off.x, 0.1f * (1.0f - ImGui::GetIO().DeltaTime));
        it_anim->second.g = ImLerp(it_anim->second.g, selected ? c_on.y : c_off.y, 0.1f * (1.0f - ImGui::GetIO().DeltaTime));
        it_anim->second.b = ImLerp(it_anim->second.b, selected ? c_on.z : c_off.z, 0.1f * (1.0f - ImGui::GetIO().DeltaTime));
    }

    /*RenderTextClipped(text_min, text_max, label, NULL, &label_size, style.SelectableTextAlign, &bb);*/
    window->DrawList->AddText(text_min + ImVec2(3,0), (ImColor)ImVec4(it_anim->second.r, it_anim->second.g, it_anim->second.b, it_anim->second.text), label);
